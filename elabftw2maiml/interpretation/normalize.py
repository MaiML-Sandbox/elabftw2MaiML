...
1. 生の値 (`RawField.value`。数値・文字列いずれもありうる) から、数値部分と
   単位表記を分離する:
     ...
     - `200` (既に数値) -> 200, (同上)
     - `"+200"`     -> 200, (先頭の`+`符号。コードレビュー 2026-09-17 5.2対応)
     - `"1,234"`    -> 1234, (桁区切りカンマ。3桁ごとの区切りのみ対応)
     - `"1.5e-3"`   -> 0.0015, (指数表記)
...
# という形に緩く分離する。数値と単位の間の空白は許容する ("200 kV"/"200kV"
# いずれも対応)。
#
# コードレビュー (2026-09-17) 5.2対応: 従来は `-?\d+(?:\.\d+)?` のみ対応しており、
# 以下のような、eLabFTWで実際に入力され得る数値表記が「正規化できない値」
# として扱われ、自動反映されず`unclassified`に回っていた。
#   - 桁区切りカンマ: "1,234" / "1,234.5"
#   - 先頭の `+` 符号: "+200"
#   - 指数表記: "1.5e-3" / "2.5E+10"
# 桁区切りカンマは、3桁ごとに正しく区切られている場合のみ数値として認識する
# (`\d{1,3}(?:,\d{3})+` の形。"1,23"のような不正な区切りは数値として認識しない
# ため、単位分離にも失敗し、従来通り「正規化できない値」として扱われる)。
...
def parse_numeric_with_unit(
    ...
    try:
        # 桁区切りカンマ ("1,234") はDecimalがそのままでは解釈できないため、
        # 正規表現で桁区切りとして妥当だと確認した上で除去してから変換する
        # (コードレビュー 2026-09-17 5.2対応)。
